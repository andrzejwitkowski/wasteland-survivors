use crate::components::{
    animated_model::{AnimatedModel, AnimationClip, PlayAnimationMessage},
...
pub fn init_animated_player_model(
    ...
    if let Ok(player) = player_query.single() {
        ...
        let walk_clip =
            AnimationClip { index: AnimationNodeIndex::new(0), name: "walk".to_string() };

        let run_clip = AnimationClip { index: AnimationNodeIndex::new(1), name: "run".to_string() };
        ...
        commands
            ...
            .insert(AnimatedModel { model_handle, animation_graph: graph_handle, animations });
...
pub fn handle_player_input(
    ...
    if let Ok(player) = player_query.single() {
        if keyboard.just_pressed(KeyCode::Digit1) {
            animation_events
                .write(PlayAnimationMessage { entity: player, animation_name: "walk".to_string() });
        }

        if keyboard.just_pressed(KeyCode::Digit2) {
            animation_events
                .write(PlayAnimationMessage { entity: player, animation_name: "run".to_string() });
        }
    }
}
